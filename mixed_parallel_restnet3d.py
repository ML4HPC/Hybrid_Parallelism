...
from  multi_input_resnet3d import *
from resnet3d import Bottleneck3d, conv1x1_3d, conv3x3_3d
...
def main():
    ...
    local_size = 1#(int)(os.environ['SLURM_NTASKS_PER_NODE'])   # wsize per-node
    ...
    if rank >=  0: ## dummpy if statement
        ...
        #devices = [(rank % local_size)*2, (rank % local_size)*2+1]
        model = SixInputMultiOutputResNet3d(Bottleneck3d,[3, 4, 6, 3], devices)
        ...
        for i in range(args.epoch):
            train(model, args.epoch, train_loader, valid_loader, optimizer, args.output_dir, devices)
